...
function startBoardDrag(event) {
  if (!running || !piece || event.pointerType === "mouse") return;
...
function dragBoard(event) {
  ...
  const horizontalStep = Math.max(18, cell * 0.72);
  while (Math.abs(dragState.carryX) >= horizontalStep) {
    move(dragState.carryX > 0 ? 1 : -1);
    dragState.carryX += dragState.carryX > 0 ? -horizontalStep : horizontalStep;
  }

  const verticalStep = Math.max(20, cell * 0.82);
  while (dragState.carryY >= verticalStep) {
    softDrop(true);
    dragState.carryY -= verticalStep;
  }
}
...
boardCanvas.addEventListener("pointerdown", startBoardDrag);
boardCanvas.addEventListener("pointermove", dragBoard);
boardCanvas.addEventListener("pointerup", stopBoardDrag);
boardCanvas.addEventListener("pointercancel", stopBoardDrag);
