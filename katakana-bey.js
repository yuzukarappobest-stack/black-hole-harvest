...
const PRODUCT_IMAGES = {
  ...
  "ヘルズサイズ": { src: "assets/bey/bx-lineup-1.jpg", x: 164, y: 263, size: 126 },
  "ウィザードアロー": { src: "assets/bey/bx-lineup-1.jpg", x: 299, y: 263, size: 126 },
  "ナイトシールド": { src: "assets/bey/bx-lineup-1.jpg", x: 435, y: 263, size: 126 },
...
const letters = QUESTIONS.map((question) => question.letter);
const questionByLetter = new Map(QUESTIONS.map((question) => [question.letter, question]));
const similarGroups = [
  ["ア", "イ", "ウ", "エ", "オ"], ["カ", "キ", "ク", "コ"], ["サ", "シ", "ス", "セ", "ソ"],
  ["タ", "チ", "テ", "ト"], ["ナ", "ニ", "ノ"], ["ハ", "フ", "ヘ", "ホ"], ["マ", "ミ", "ム", "メ"],
  ["ユ", "ワ", "ン", "ラ", "リ", "ル", "レ", "ロ"],
];
...
const pictureCrop = document.getElementById("pictureCrop");
const pictureCropImage = document.getElementById("pictureCropImage");
...
function pickLetter() {
  ...
  letterCard.classList.toggle("product-word", currentMode === "listen");
  letterCard.textContent = currentMode === "listen" ? maskedProductName(currentLetter) : currentLetter;
  updatePictureHint();
  showFeedback(" ", "");
  answered = false;
...
function updatePictureHint() {
  const image = questionByLetter.get(currentLetter)?.image;
  pictureEmoji.classList.add("hidden");
  pictureImage.classList.add("hidden");
  pictureImage.removeAttribute("src");
  pictureCrop.classList.toggle("hidden", !image);
  if (!image) return;

  const sourceWidth = 591;
  const safeInset = 6;
  const cropSize = image.size - safeInset * 2;
  const scale = 1 / cropSize;
  pictureCropImage.src = image.src;
  pictureCropImage.style.width = `${sourceWidth * scale * 100}%`;
  pictureCropImage.style.left = `${-(image.x + safeInset) * scale * 100}%`;
  pictureCropImage.style.top = `${-(image.y + safeInset) * scale * 100}%`;
}
...
function checkChoice(choice, button) {
  ...
  letterCard.classList.remove("product-word");
...
function buildListenPromptParts(letter) {
  const cue = questionByLetter.get(letter)?.word || letter;
