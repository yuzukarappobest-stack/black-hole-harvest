...
function setMode(mode) {
  ...
  prompt.textContent = mode === "listen" ? "ことばをきいてえらぼう" : "よんでみよう";
...
function pickLetter() {
  ...
  currentWord = listenWords[currentLetter] || currentLetter;
  letterCard.classList.remove("word-answer");
...
function renderChoices() {
  const choices = buildWordChoices(currentLetter);
...
function buildWordChoices(letter) {
  const group = getSeriesLetters(letter);
  return shuffleArray(group.map((item) => currentWord.replace(letter, item)));
}
...
function checkChoice(choice, button) {
  ...
  prepareAudio();
  answered = true;
  const isCorrect = choice === currentWord;
  letterCard.classList.add("word-answer");
  letterCard.textContent = currentWord;
