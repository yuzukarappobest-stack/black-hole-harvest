...
const REWARDS = [["black-hole", "ブラックホール", "index.html"], ["kingfisher", "カワセミ", "kingfisher.html"], ["tetris", "テトリス", "tetris.html"], ["butterfly", "ちょうちょ", "butterfly.html"], ["meteor", "いんせき", "meteor.html"], ["race", "レース", "race.html"], ["gem", "ほうせき", "gem.html"], ["pillbug", "ダンゴムシ", "pillbug.html"], ["dango-shot", "ダンゴショット", "dango-shot.html"], ["planet-catch", "惑星", "planet-catch.html"], ["shooting-star", "流れ星", "shooting-star.html"], ["frog-jump", "カエルジャンプ", "frog-jump.html"], ["fruit-rush", "フルーツラッシュ", "fruit-rush/"], ["space-blaster", "スペースブラスター", "space-blaster/"], ["kabuto-sumo", "カブトムシ相撲", "kabuto-sumo/"], ["kaiju-defense", "かいじゅうぼうえいせん", "kaiju-defense/"]];
const problemText = $("problemText"), categoryLabel = $("categoryLabel"), answerDisplay = $("answerDisplay"), correctCount = $("correctCount"), feedback = $("feedback"), submitButton = $("submitButton"), decimalButton = $("decimalButton"), backspaceButton = $("backspaceButton"), clearAnswerButton = $("clearAnswerButton"), clearScratchButton = $("clearScratchButton"), scratchCanvas = $("scratchCanvas"), completePanel = $("completePanel"), rewardGrid = $("rewardGrid"), againButton = $("againButton");
...
const buildProblems = () => {
  const problems = [];
  [[1.2,4,"リボン","m"],[2.5,3,"ジュース","L"],[1.6,5,"色紙","m"],[3.4,2,"ひも","m"],[0.8,7,"りんご","kg"],[2.3,4,"テープ","m"],[1.5,6,"小麦粉","kg"],[4.2,3,"布","m"],[0.6,9,"リボン","m"],[2.7,5,"花のたね","g"],[1.8,4,"テープ","m"],[3.5,2,"水","L"],[0.9,8,"小麦粉","kg"],[2.4,6,"ひも","m"],[1.25,4,"牛乳","L"]].forEach(([a,b,item,unit]) => problems.push(make("小数のかけ算", `同じものを ${b}こ作ります。1こに ${item}を ${a} ${unit}ずつ使います。\n全部で 何 ${unit}使いますか。`, a * b)));
...
function resetLesson() { queue = shuffle(PROBLEMS).slice(0, CONFIG.requiredCorrect); correct = 0; correctCount.textContent = "0"; completePanel.classList.add("hidden"); sessionStorage.removeItem(REWARD_TOKEN_KEY); nextProblem(); }
...
function submit() { if (!answer) return; const correctAnswer = Number(currentProblem.answer), entered = Number(answer); if (Number.isFinite(entered) && Math.abs(entered - correctAnswer) < .00001) { correct += 1; correctCount.textContent = correct; feedback.textContent = "せいかい！"; feedback.className = "feedback good"; if (correct >= CONFIG.requiredCorrect) { window.setTimeout(showComplete, CONFIG.nextDelayMs); return; } } else { feedback.textContent = `ざんねん！ こたえは ${currentProblem.answer}`; feedback.className = "feedback bad"; } window.setTimeout(nextProblem, CONFIG.nextDelayMs); }

function showComplete() { sessionStorage.setItem(REWARD_TOKEN_KEY, `${Date.now()}-${Math.random()}`); window.location.replace("game-menu.html"); }
function renderRewards() { REWARDS.forEach(([id, label, href]) => { const button = document.createElement("button"); button.type = "button"; button.textContent = label; bind(button, () => { if (!sessionStorage.getItem(REWARD_TOKEN_KEY)) { window.location.replace("learn.html"); return; } sessionStorage.removeItem(REWARD_TOKEN_KEY); sessionStorage.setItem(`${MINI_GAME_ACCESS_PREFIX}${id}`, "1"); sessionStorage.setItem("miniGameReturnUrl", "learn.html"); window.location.replace(href); }); rewardGrid.appendChild(button); }); }
function enforceRewardTokenOnRestore() { if (!completePanel.classList.contains("hidden") && !sessionStorage.getItem(REWARD_TOKEN_KEY)) window.location.replace("learn.html"); }
...
renderRewards(); window.addEventListener("pageshow", enforceRewardTokenOnRestore); resizeScratch(); resetLesson();
