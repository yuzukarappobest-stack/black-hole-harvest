(() => {
  ...
  function canUseHandCard(side,inst){
    ...
    if(c.type==='enhance'){
      ...
      if(c.effect==='blackSilverThread')return effectiveCardCost(side,inst)<=ss.cost&&ss.discard.some(x=>def(x).type==='insect');
    ...
    if(c.effect==='youngReincarnation')return fieldActive(side).some(fc=>!fieldDef(fc).name.includes('（幼虫）')&&ss.discard.some(x=>def(x).type==='insect'&&def(x).name===matchingLarvaName(fieldDef(fc).name)));
    if(c.effect==='ghostSwap')return legalOpp.length>0&&faceUpBait(opp).some(x=>def(x).type==='insect');
    if(c.effect==='polarEvolution')return fieldActive(side).some(fc=>fieldDef(fc).name.includes('（幼虫）')&&ss.discard.some(x=>def(x).type==='insect'&&def(x).name===baseAdultName(fieldDef(fc).name)));
  ...
  async function handleInsectEntered(side,fc){
    ...
    if(p.type==='reinforcedCarapace'){
      ...
      const choices=sideObj(side).discard.filter(x=>def(x).type==='enhance'&&Number(def(x).cost||0)<=3&&canAttachEnhancement(fc,x));
    ...
    if(p.type==='locustHarvest'){
      const choices=sideObj(side).discard.filter(x=>def(x).type==='enhance'&&Number(def(x).cost||0)===0);
      if(!choices.length)return;
  ...
  function leaveFieldToHand(side,fc){
    ...
    if(rawFieldPassive(fc)?.type==='silenceAll')enforceAllAttachmentLegality();
    return true;
  }
  ...
  async function resolveSpiritBody(side,fc,attacker){
    if(!canSpiritBody(side,fc))return false;
    let use=true;
    ...
    if(side==='player'){
      if(hasArmor){
        const mode=await choose([
          ...
          {value:null,title:'防がない',detail:'そのまま破壊される'}
        ],'破壊を防ぐ効果を選んでください。','破壊置換');
        if(mode==='armor'){consumeArmorSynchronously(side,fc);return true;}
        use=mode==='spirit';
      }else use=await confirmYesNo('＜霊体＞で縄張りを2枚引き、破壊を防ぎますか？','霊体');
    }else{
      if(hasArmor){consumeArmorSynchronously(side,fc);return true;}
      use=sideObj(side).territory.length>=3;
    }
    if(!use)return false;
    await takeTerritory(side,false,{effectDraw:true,forceDraw:true,spiritDraw:true});
    await takeTerritory(side,false,{effectDraw:true,forceDraw:true,spiritDraw:true});
    if(!sideObj(side).field.includes(fc))return true;
    fc.damage=fc.persistentDamage||0;
    log(`＜霊体＞ 「${fieldDef(fc).name}」は破壊されず、回復できるダメージを回復した。`);
    if(fc.damage>=maxHp(fc))await attemptDestroyFieldCard(side,fc,'attack',attacker);
    return true;
  }
  ...
  async function attemptDestroyFieldCard(side,fc,reason,attacker){
    ...
    if(reason==='attack'&&canSpiritBody(side,fc)){
      const prevented=await resolveSpiritBody(side,fc,attacker);
  ...
  function destroyFieldCard(side,fc,reason,attacker,options={}){
    ...
    fc.preDestroyEnhanceUids=sideObj(owner).discard.filter(x=>def(x).type==='enhance').map(x=>x.uid);
    ss.field=ss.field.filter(x=>x!==fc);
    ...
    if(beforePassive?.type==='silenceAll')enforceAllAttachmentLegality();
    return true;
  }
  ...
  async function resolveAttackDestructionReaction(defenderSide,target,attacker){
    ...
    if(p?.type==='discardEnhanceRecover'){
      ...
      const choices=sideObj(owner).discard.filter(x=>allowed.has(x.uid)&&def(x).type==='enhance');
      if(choices.length){
  ...
  async function resolveSpell(side,inst,c){
    ...
    if(c.effect==='recoverInsect'){
      ...
      if(s.discard.length<2)return false;
      const first=await chooseOwnedInstance(side,'裏向きのエサにする捨て札（1枚目）を選んでください。',s.discard);if(!first)return false;
      const second=await chooseOwnedInstance(side,'裏向きのエサにする捨て札（2枚目）を選んでください。',s.discard.filter(x=>x.uid!==first.uid));if(!second)return false;
      if(!paySpell(side,inst,c))return false;
      ...
      let choices=s.discard.filter(x=>def(x).type==='enhance'&&/(甲冑|贋作)/.test(def(x).name));if(!choices.length)return false;
      if(!paySpell(side,inst,c))return false;
      let moved=0;
  ...
  async function dealDamage(targetSide,target,amount,ctx={}){
    ...
    if(ctx.kind==='spell'&&ctx.sourceSide&&ctx.sourceSide!==targetSide){
      if(spellDamageDestroyImmune(targetSide,target)){
        ...
        const same=spellName&&['player','cpu'].some(owner=>sideObj(owner).discard.some(x=>def(x).type==='spell'&&def(x).name===spellName));
  ...
  async function applyAttackDamage(side,fc,target,attack,base){
    ...
    const noWeak=hasAttachment(target,'noWeakness')||hasAttachment(target,'longhornJaw')||(passiveOfField(target)?.type==='whiteShell'&&target.whiteShellTurn===state.turnSeq);
  ...
  function canDragonUseCardOnHost(side,inst,host){
    const c=def(inst);
    if(c.type==='enhance')return !['summonWithAttachment','silverThread','blackSilverThread'].includes(c.effect)&&canAttachEnhancement(host,inst);
  ...
  async function dragonUseFreeOnHost(side,host){
    ...
    if(c.effect==='singleAttack500')host.turnAttackBonus=(host.turnAttackBonus||0)+500;
      ...
      const avail=faceUpBait(side).filter(x=>canSwordDanceAttach(side,x)&&canAttachEnhancement(host,x));
      if(avail.length){
        const first=await chooseOwnedInstance(side,'オオカレエダカマキリにつける強化カードを選んでください。',avail);
        if(first){removeInstance(sideObj(side).bait,first);host.attachments.push(first);await configureAttachedCard(side,host,first);}
      }
    }
  ...
  async function resolveDestroyedAttackTarget(side,fc,target,attack){
    ...
    if(attack.effect==='ovipositWasp'){
      const choices=sideObj(side).discard.filter(x=>def(x).type==='insect'&&/バチ/.test(def(x).name));
      if(choices.length){
  ...
  async function performAttack(side,fc,attack){
    ...
    if(attack.effect==='paradiseEclosion'){
      ...
      const adults=sideObj(side).discard.filter(x=>def(x).name==='ゴクラクトリバネアゲハ');
      if(adults.length){
  ...
  async function takeTerritory(side,isDirect,options={}){
    ...
    if(c.type==='enhance'&&c.territorySpecial&&c.effect==='blackSilverThread'&&ss.discard.some(x=>def(x).type==='insect')){
